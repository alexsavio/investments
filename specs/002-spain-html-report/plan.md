...
- ~~The Spanish processor never reads `broker_statement.cash_grants`.~~ Fixed in its own PR. Note
  for anyone re-reading the finding: only the Sber reader builds a `CashGrant` today, so an
  Interactive Brokers statement could never have reached the dropped path. The inconsistency with
  Germany was real, the exposure was not.
