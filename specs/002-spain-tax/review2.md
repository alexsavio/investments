...
- **F1 — Credit: per-dividend treaty limb, net of the exemption.** Status: ✅ Done (`919c9c66`)
...
- **F2 — Overlap guard vs unpriced years.** Status: ✅ Done (`2e8936a3`)
...
- **F3 — Interest reversals are income corrections, not margin interest.** Status: ✅ Done (`23fd420f`)
...
- **F4 — Disjoint compensation reporting.** Status: ✅ Done (`214bf107`)
...
- **F5 — Wash-sale definitiveness measured on the blocked shares.** Status: ✅ Done (`3a4888ba`)
...
- **F6 — Anti-abuse clause: homogeneity key + unlisted-variant disclosure.** Status: ✅ Done (`cae4b61b`)
...
- **F7 — Documentation corrections.** Status: ✅ Done (`f92c9725`) — one commit:
...
- **F8 — CSV contract refresh.** Status: ✅ Done (`8ff18098`) — bring `specs/002-spain-tax/contracts/csv-output.md` up to the emitted format:
...
- **F9 — Mechanical code batch.** Status: ✅ Done (`0138a636`) — one commit:
...
**Date**: 2026-08-11 · Branch `002-spain-tax`, ten tasks on top of `52d16f4e`.

### Commits

| Task | Commit |
|---|---|
| F1 per-dividend treaty limb, net of the exemption | `919c9c66` |
| F2 carried-in deferrals vs unpriced statement years | `2e8936a3` |
| F3 interest reversals net against income | `23fd420f` |
| F4 disjoint own-group / crossed compensation rows | `214bf107` |
| F5 wash-sale definitiveness on the blocked shares | `3a4888ba` |
| F6 anti-abuse rule on the homogeneity key | `cae4b61b` |
| F7 documentation corrections | `f92c9725` |
| F8 CSV contract refresh | `8ff18098` |
| F9 low-severity batch | `0138a636` |
