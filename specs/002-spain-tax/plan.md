...
- **S5**: extract `format_eur`/`round_eur` from `tax_statement/germany/mod.rs` into `src/tax_statement/eur.rs` (`pub(crate)`); `germany/mod.rs` re-exports so zero call sites change. The one-formatter invariant (commit `0a72a692`) becomes cross-jurisdiction. Spanish forms also use 2-dp EUR half-away-from-zero (`// TODO(verify)`).
...
- **S2 — Fixture harness.** Status: ✅ Done (`1b20a157`)
...
- **S3 — Jurisdiction.** Status: ✅ Done (`920d7723`)
...
- **S4 — Config block.** Status: ✅ Done (`f22de522`)
...
- **S5 — Shared EUR formatter.** Status: ✅ Done (`575f8df6`)
...
- **S6 — SavingsScale.** Status: ✅ Done (`8f49a92b`)
...
- **S7 — Coefficients.** Status: ✅ Done (`1c3409a5`)
...
- **S8 — Statement + trades + routing.** Status: ✅ Done (`531271ba`)
...
- **S9 — RCM income.** Status: ✅ Done (`f23d1263`)
...
- **S10 — FX.** Status: ✅ Done (`172f3af9` refactor, `4fc8fafc` feature)
...
- **S11 — Compensation + credit + totals.** Status: ✅ Done (`040042b6`)
...
- **S12 — Output.** Status: ✅ Done (`880fe00a`)
...
- **S13 — Sell simulation.** Status: ✅ Done (`adc683d2`)
...
- **S14 — Primitives.** Status: ✅ Done (`b5ee2117`)
...
- **S15 — Deferral.** Status: ✅ Done (`40a56fae`)
...
- **S16 — Reintegration.** Status: ✅ Done (`97b5e1d0`)
...
- **S17 — Regime end-to-end.** Status: ✅ Done (`53072f1f`)
...
- **S18 — Cross-offset.** Status: ✅ Done (`c225358d`)
...
numbers. For the branch as it actually stands, read those two files and `git log 16c4d038..HEAD`
