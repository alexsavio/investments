...
| FIFO | **art. 47.2** — confirmed; current wording also covers "criptoactivos fungibles del mismo tipo". Official foral guidance: sole-owner and jointly-held holdings run **separate** FIFO queues (out of scope, doc note) | **art. 37.2** — confirmed, scoped to art. 37.1 letters a)/b)/c) |
...
- **S2 — Fixture harness.** Status: TODO
...
- **S3 — Jurisdiction.** Status: TODO — localities + TaxJurisdiction + `get_tax_country` + widened match arms (`trades.rs:200`, `interest.rs:92`). Test: `jurisdiction: spain` yields `Country { code: "ES", currency: "EUR" }` with ECB rate source; unknown-year `SavingsScale` error message named.
  Commit: `feat(spain-tax): add Spain jurisdiction with ECB rates and analysis approximation`
- **S4 — Config block.** Status: TODO — `SpanishTaxConfig` + accessors + `LossLedger::from_config` validation (origin ≥ filing year rejected; origin < filing−4 rejected with "expired" message). serde tests mirror `taxes/mod.rs:265-390` patterns.
  Commit: `feat(spain-tax): add taxes.spain config block`
- **S5 — Shared EUR formatter.** Status: TODO — extraction as specified; German tests stay green untouched.
  Commit: `refactor(tax-statement): move the EUR formatter to a shared module`

### Phase 2 — Gipuzkoa 2026 core

- **S6 — SavingsScale.** Status: TODO — all shipped tables + `average_rate`; rstest vectors from Appendix A incl. the cuota-íntegra golden vectors and bracket-edge cases.
  Commit: `feat(spain-tax): add savings-base scales for Gipuzkoa and Territorio Común`
- **S7 — Coefficients.** Status: TODO — shipped DF tables (from S1), override map, clamping, unknown-year error.
  Commit: `feat(spain-tax): add Gipuzkoa actualization coefficients`
- **S8 — Statement + trades + routing.** Status: TODO — `SpanishTaxStatement`, `compute_tax_year`, `process_trades` with per-lot coefficients, `generate_spanish_tax_statement` routing. Un-ignore `fifo`; add `coefficients` fixture (same trades run under both regimes differ exactly by the coefficient delta).
  Commit: `feat(spain-tax): compute capital gains with per-lot FIFO and coefficients`
- **S9 — RCM income.** Status: TODO — dividends, interest, **regime-dependent** custody-fee deduction (S1: deductible under Común art. 26.1.a, **not** deductible under Gipuzkoa art. 39), per-entry treaty-capped credit candidates. `income` fixture (US dividend 30% WHT, broker interest, custody fee) asserted under **both** regimes so the deductibility split is covered.
  Commit: `feat(spain-tax): process dividends, interest and deductible custody fees`
- **S10 — FX.** Status: TODO — two commits: `refactor(tax-statement): share the foreign-currency FIFO engine` then `feat(spain-tax): tax FX conversion gains in the savings base` (`fx_gain` fixture; borrowed-balance → `fx_borrowed_review`).
- **S11 — Compensation + credit + totals.** Status: TODO — `LossLedger`, `compensate_savings_base` (Gipuzkoa path: no cross-offset), `calculate_totals`, year-level credit, `net_tax_due`. `loss_carryforward` fixture (per-group origin-year ledgers, oldest-first, expiry boundary: origin = year−4 usable, older rejected); completes `income` assertions.
  Commit: `feat(spain-tax): compensate savings-base groups with 4-year loss ledgers`
- **S12 — Output.** Status: TODO — CSV formatter + contract doc `specs/002-spain-tax/contracts/csv-output.md` + console summary + Modelo 109 mapping with `# WARNING` caveats + next-year config block printout.
  Commit: `feat(spain-tax): emit the Spanish CSV statement and Modelo 109 summary`
- **S13 — Sell simulation.** Status: TODO — `SpanishTaxSimulation` in `simulate-sell`.
...
- **S14 — Primitives.** Status: TODO — window arithmetic (`chrono::Months`), instrument identity, matching helpers; rstest month-end/leap edges (Dec-31 sale, Jan/Feb windows).
  Commit: `feat(spain-tax): add valores-homogéneos window matching`
- **S15 — Deferral.** Status: TODO — replay + deferral in the pipeline. Fixtures `wash_sale_after`, `wash_sale_before` (repurchase 1 month before, shares still held).
  Commit: `feat(spain-tax): defer losses on repurchased homogeneous securities`
- **S16 — Reintegration.** Status: TODO — release on disposal, `carry_out`, opening `deferred_losses`. Fixture `wash_sale_multi_lot` (partial repurchase → proportional deferral; pro-rata multi-lot release; surviving carry-out asserted).
