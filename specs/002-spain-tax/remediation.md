...
- **R1 — Margin interest must not reduce RCM.** Status: ✅ Done (`aef3ec15`)
...
- **R2 — Wash-sale carry-out: snapshot at filing-year end and guard config overlap.** Status: ✅ Done (`eb7fe666`)
...
- **R5 — Post-split units in the wash-sale replay.** Status: ✅ Done (`0004d0ac`)
...
- **R10 — Gipuzkoa €1,500 dividend exemption (art. 9.24): verify, then implement or refute.** Status: ✅ Done (`617ec208`) — verdict **CONFIRMED, implemented**.
...
|---|---|---|
| R1 margin interest | `aef3ec15` | Paid interest no longer reduces RCM |
| R2 carry-out snapshot + overlap guard | `eb7fe666` | A carry-out is no longer emptied by post-year-end disposals; a double-deducting config is refused |
| R4 Común two-phase compensation | `14bc4502` | Común bases change wherever prior-year balances and a current-year cross-offset meet; Gipuzkoa bit-identical |
| R3 credit net base + 2-dp rate | `aafff144` | Credits shrink where expenses or compensation reduced the foreign income; the rate is rounded before multiplying |
| R5 post-split units | `0004d0ac` | Deferrals across a split match the right fraction |
| R6 coefficient validation | `5d9433eb` | A nonsensical override is refused instead of silently pricing |
| R7 fee/ledger-only years | `9ded6408` | Those years now produce a statement |
| R8 casillas | `f8c877d8` | Foreign WHT off the retenciones box; Modelo 100 numbered |
| R9 definitive-disposal releases | `a6365e0e` | A chained sale no longer integrates a loss that is still blocked |
| R10 dividend exemption | `617ec208` | **Largest effect**: −€1,500/year of Gipuzkoa dividend income, and the matching loss of credit |
| R11 grants + corporate actions | `b11826c1` | Reporting only |
| R12 mechanical batch | `e73e8e37` | No panics on an unpriced filing year; real coefficient faults propagate; markers cited |
