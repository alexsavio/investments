    ...
    fn process_trades(&mut self, tax_calculator: &TaxCalculator, mut tax_statement: Option<&'a mut TaxStatement>) -> EmptyResult {
        ...
        for trade in &self.broker_statement.stock_sells {
            ...
            match broker_jurisdiction {
                ...
                // This matches the *broker's* jurisdiction, and no broker files under Germany or
                // Spain — both are filer jurisdictions whose statements are generated by
                // `tax_statement::{germany,spain}`, which return before this code runs. The arms
                // exist to keep the match exhaustive.
                Jurisdiction::Germany | Jurisdiction::Spain => {
                    self.has_income_to_declare = true;
                },
