...
impl CsvFormatter {
    ...
    fn write_summary_rows<W: Write>(
        ...
        row(
            writer,
            "SUMMARY_GYP_FX",
            forms::fx_summary_label(statement.regime),
    ...
    /// Emit the casillas of the filer's own return, from the mapping the report also renders.
    fn write_modelo_boxes<W: Write>(
        writer: &mut W,
        statement: &SpanishTaxStatement,
    ) -> GenericResult<()> {
        let mapping: FormMapping = forms::form_mapping(statement);

        writeln!(writer)?;
        writeln!(writer, "# {}", mapping.title)?;
        ...
        for form_box in &mapping.boxes {
            writeln!(
                ...
                form_box.key,
                form_box.label(),
                Self::format_decimal(form_box.value)
            )?;
        }

        for line in &mapping.footer_lines {
            writeln!(writer, "# {line}")?;
        }

        Self::write_withholding_warning(writer, statement, &mapping.withholding_casilla)?;
